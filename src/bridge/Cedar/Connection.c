...
void ConnectionReceive(CONNECTION *c, CANCEL *c1, CANCEL *c2)
{
	...
	// CRITICAL: Log FIRST to verify this override is being used!
	LOG_INFO("ConnRecv", "📞 ConnectionReceive #%llu CALLED! c=%p", call_count, c);
	...
	if (c == NULL)
	{
		LOG_ERROR("ConnRecv", "❌ Connection is NULL!");
		return;
	}

	if (call_count <= 10 || (call_count % 50) == 0) {
		LOG_INFO("ConnRecv", "📊 Call #%llu Protocol=%u ReceivedBlocks=%u", 
			call_count, c->Protocol, c->ReceivedBlocks ? c->ReceivedBlocks->num_item : 0);
	}

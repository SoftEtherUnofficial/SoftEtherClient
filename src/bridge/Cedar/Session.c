...
void SessionMain(SESSION *s)
{
	...
	LOG_INFO("SESSION", "🚀 SessionMain STARTED for session: %s", s ? s->Name : "(null)");
	
	// Validate arguments
	if (s == NULL)
	{
		LOG_ERROR("SESSION", "❌ CRITICAL: Session pointer is NULL!");
		return;
	}
	...
	LOG_INFO("SESSION", "📡 Calling pa->Init... pa=%p", pa);
	
	if (pa->Init(s) == false)
	{
		// Initialization Failed
		LOG_ERROR("SESSION", "❌ pa->Init FAILED!");
	...
	LOG_INFO("SESSION", "✅ pa->Init SUCCESS!");
	...
	LOG_INFO("SESSION", "🔁 Entering main loop...");
	LOG_INFO("SESSION", "   pa=%p, ServerMode=%d, pa_inited=%d", pa, s->ServerMode, pa_inited);
	...
	while (true)
	{
		...
		if (loop_count <= 5 || loop_count % 100 == 0) {
			LOG_INFO("SESSION", "🔄 Loop #%llu", loop_count);
		}
		...
		// Add the packet to be transmitted to SendBlocks by acquiring from PacketAdapter
		{
			...
			{
				if (loop_count <= 5 || i < 3) {
					LOG_INFO("SESSION", "📦 Packet size=%u loop=%llu #%u", packet_size, loop_count, i);
				}
				BLOCK *b;
...
//					WHERE;
		...
		// Time-out decision
		if (loop_count <= 5) {
			LOG_INFO("SESSION", "🔍 Loop #%llu end: pa_fail=%d SessionTimeOuted=%d", loop_count, pa_fail, s->SessionTimeOuted);
		}

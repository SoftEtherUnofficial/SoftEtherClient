//! iOS TUN Adapter
//!
//! This module integrates with iOS Network Extension (PacketTunnelProvider)
//! and uses deps/taptun for L2↔L3 protocol translation.
//!
//! Architecture:
//!   PacketTunnelProvider (Swift)
//!     ↓ file descriptor
//!   iOS TUN Adapter (this file)
//!     ↓ L2↔L3 translation
...
comptime {
    if (builtin.os.tag != .ios) {
        @compileError("This module is only for iOS");
    }
}

/// iOS VPN Device using Network Extension
pub const iOSVpnDevice = struct {
    ...
    /// Initialize iOS VPN device
    pub fn init(
        allocator: std.mem.Allocator,
        fd: std.posix.fd_t,
        mac_address: [6]u8,
    ) !*iOSVpnDevice {
        const self = try allocator.create(iOSVpnDevice);
    ...
    /// Clean up resources
    pub fn deinit(self: *iOSVpnDevice) void {
    ...
    /// iOS NEPacketTunnelFlow provides IP packets directly (no Ethernet headers)
    ...
    /// iOS NEPacketTunnelFlow expects IP packets directly (no Ethernet headers)
    /// Returns number of bytes written
    pub fn write(self: *iOSVpnDevice, data: []const u8) !usize {
    ...
    /// Most iOS usage won't need this, but included for compatibility
    pub fn ipToEthernet(self: *iOSVpnDevice, ip_packet: []const u8) ![]u8 {
        return try self.translator.ipToEthernet(ip_packet);
    }

    /// Convert Ethernet frame to IP packet (if needed for SoftEther)
    /// Most iOS usage won't need this, but included for compatibility
    pub fn ethernetToIp(self: *iOSVpnDevice, eth_frame: []const u8) !?[]u8 {
        return try self.translator.ethernetToIp(eth_frame);
    }

    /// Get learned IP address (if any)
    pub fn getLearnedIp(self: *iOSVpnDevice) ?u32 {
        return self.translator.our_ip;
    }

    /// Get learned gateway MAC (if any)
    pub fn getGatewayMac(self: *iOSVpnDevice) ?[6]u8 {
        return self.translator.gateway_mac;
    }

    /// Get statistics
    pub fn getStats(self: *iOSVpnDevice) Stats {
        return .{
...
/// Opaque handle for iOS VPN device
pub const iOSVpnDeviceHandle = ?*anyopaque;

/// Create iOS VPN device from file descriptor
export fn ios_vpn_device_create(
    fd: c_int,
    mac_address: [*]const u8,
) callconv(.c) iOSVpnDeviceHandle {
    const allocator = std.heap.c_allocator;

    var mac: [6]u8 = undefined;
    @memcpy(&mac, mac_address[0..6]);

    const device = iOSVpnDevice.init(
        allocator,
        @intCast(fd),
        mac,
    ) catch return null;
...
/// Destroy iOS VPN device
export fn ios_vpn_device_destroy(handle: iOSVpnDeviceHandle) callconv(.c) void {
    if (handle) |h| {
        const device: *iOSVpnDevice = @ptrCast(@alignCast(h));
        device.deinit();
    }
}

/// Read packet from device
export fn ios_vpn_device_read(
    handle: iOSVpnDeviceHandle,
    buffer: [*]u8,
    buffer_len: usize,
) callconv(.c) isize {
    ...
    const bytes_read = device.read(buf_slice) catch |err| {
        std.log.err("iOS device read error: {}", .{err});
        return -1;
    };
...
export fn ios_vpn_device_write(
    handle: iOSVpnDeviceHandle,
    data: [*]const u8,
    data_len: usize,
) callconv(.c) isize {
    ...
    const bytes_written = device.write(data_slice) catch |err| {
        std.log.err("iOS device write error: {}", .{err});
        return -1;
    };
...
export fn ios_vpn_device_get_stats(
    handle: iOSVpnDeviceHandle,
    out_stats: ?*iOSVpnDevice.Stats,
) callconv(.c) c_int {
...
export fn ios_vpn_device_get_learned_ip(handle: iOSVpnDeviceHandle) callconv(.c) u32 {
...
export fn ios_vpn_device_get_gateway_mac(
    handle: iOSVpnDeviceHandle,
    out_mac: [*]u8,
) callconv(.c) bool {
